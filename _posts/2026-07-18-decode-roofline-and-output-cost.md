...
> † 70B FP16 单卡放不下（§5.5），本行是 per-GPU 折算值（按 INT4 等效吞吐上调推回 FP16 单卡等价），实际部署需 TP ≥ 2。
>
...
> 算式：prefill FLOPs = 2PS + 2LHS² ≈ 145 GFLOPs/token。H100 FP16 1979 TFLOPs，MFU 45% → 6140 tok/s。租价 $2.4/h → $0.109/1M input token。
>
