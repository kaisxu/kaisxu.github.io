...
```
算术强度 = 一次推理做的浮点运算 / 一次推理从显存读出的字节数   (FLOP/byte)
```
...
注意表格里有两个完全不同的量：权重摊薄是 `2B/σ_w`（按 batch 摊权重），KV 流量是 `2g/σ_kv = 8`（与 batch 无关、与上下文长度无关）。前者解释了 batching 的红利，后者解释了为什么 batching 不能无限提升。第三节会从公式推导。
...
- **斜线段**：`T = BW · AI`（被带宽限制，斜率 = HBM 带宽 BW）。
- **水平线段**：`T = F_peak`（被算力限制，高度 = 峰值 FLOP/s）。
- **拐点**：`AI_ridge = F_peak / BW`。H100 SXM FP16 tensor core `1979 TFLOPs / 3.35 TB/s ≈ 590 FLOP/byte`。
...
- `L` 层数，`H` 隐藏维度，query 头数 `h_q`，KV 头数 `h_kv`，head dim `d = H / h_q`，分组因子 `g = h_q / h_kv`。
- 当前生成第 `o` 个 token 时，上下文长度 `T = S + o - 1`（`S` = ISL，`o-1` = 此前已生成 token）。
- `B` 同一时刻服务的独立序列数。
- 权重按 `σ_w` 字节存（FP16/BF16 = 2，FP8/INT8 = 1，FP4/INT4 = 0.5），KV cache 按 `σ_kv` 字节存（默认 FP16 = 2，FP8 = 1）。
...
- 合计：`F_token = 2P + 4·L·H·T`。

LLaMA-70B（`P ≈ 70×10⁹`，`L=80`，`H=8192`，`T=4096`）：`F_token ≈ 150.7 GFLOPs`。这跟 vLLM 在公开 benchmark 中的 140-160 GFLOPs/token 一致。

### 3.2 HBM 字节数（理想情况下，不计 cache 命中 / kernel fusion）

- 权重按 batch 摊薄：`P·σ_w / B` 字节/token。
- KV cache read：`2·L·T·H·σ_kv / g` 字节/token。
- KV cache write（新 token 写入）：`2·L·B·H·σ_kv / g` 字节/token。

```
M_token(B, T) ≈ P·σ_w / B + 2·L·T·H·σ_kv / g + 2·L·B·H·σ_kv / g
```

### 3.3 算术强度

- **线性层**：`AI_lin = (2P) / (P·σ_w/B) = 2B/σ_w`。只看权重搬运时，跟模型大小、形状都无关。
- **注意力**：

```
AI_attn = (4·L·T·H) / (2·L·T·H·σ_kv/g) = 2g/σ_kv
```
...
```
AI_agg(B) = (2P + 4·L·H·T) / (P·σ_w/B + 2·L·T·H·σ_kv/g + 2·L·B·H·σ_kv/g)
```
...
带宽限制段：`T_token_bandwidth = BW · AI_agg`。H100 3.35 TB/s × AI_agg：
...
- `F_prefill = 2P·S + 2·L·H·S²`（causal 注意力下，系数 2 与 FlashAttention 论文一致）。
- `AI_lin_prefill = 2·B·S / σ_w`：权重按 prompt 长度摊薄，S=4096 FP16 → 4096 FLOP/byte，远高于 H100 拐点。
- `AI_attn_prefill = 2·S·g / σ_kv`：注意力同样按 S 摊薄。
...
```
线性层 AI: 2B/σ_w         → 随 B 线性涨
注意力 AI: 2g/σ_kv        → 常数
聚合 AI:  加权和          → 跟 B 的关系近似线性，在远低于 ridge 时
可达吞吐: BW · AI_agg     → 跟 B 近似线性（在带宽段）
```
...
```
F_decode_total = 2P·O + 4·L·H·[O·S + O(O-1)/2]
```
...
`AI_attn = 2g/σ_kv` 跟 B 无关、跟 T 无关——这是 GPU 内存子系统的硬约束。但 GQA 把 `g` 推高，等于把"每 KV 字节对应的算力"抬上去。NVIDIA A100/H100/B100 系列 GPU 没有专门给 attention 优化的 SRAM tiling，但 FlashAttention / FlashDecoding 这类软件 trick 在 SRAM 上复用 K/V tile——本质上是 **用 on-chip SRAM 把 HBM 带宽压力往上一层"截流"**。
...
如果按上述纯 GPU 路径算，物理下限基本就到 $1/1M token 量级。再往下走必须换内存层级：

| 路线 | 代表产品 | 思路 | 现状 (2026-07) |
|---|---|---|---|
| **PIM** | SK Hynix AiM (GDDR6 PIM) | 计算在 DRAM 内完成，权重不搬 | GDDR6 AiM 已量产；HBM-PIM 第二代在研 |
| **APU** | AMD MI300A | CPU/GPU 共享 HBM，省一次拷贝 | 已商用，ROCm + vLLM 跑 LLaMA-70B 可用 |
| **LPU** | Groq LPU、Cerebras WSE | 用 SRAM 而非 DRAM，带宽数十 TB/s | Groq 已商用 LPU Inference |

这些方案的"理论下限"比 GPU 再低 3-10×，但要看软件栈成熟度才能进入大规模生产。

参考：[SK Hynix AiM 白皮书](https://www.skhynix.com/sustainability/governance.do)、[AMD MI300A 产品页](https://www.amd.com/en/products/accelerators/instinct/mi300/mi300a.html)、[Groq LPU 架构博文](https://wow.groq.com/2024-01-08-why-llama-2-runs-blazingly-fast-on-groq-lpu-inference-engine/)。
