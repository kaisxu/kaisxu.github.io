...
> **图 1**：下文 §3.5 末尾的 roofline 图把 H100/H200/B200 三条 BW 斜线、FP16 算力墙、四个 decode 工作点（B=1/8/32/128）和一个 prefill 工作点画在同一张图上。
...
- "4-8× 价格差是物理常数"——纯硅成本差 batched serving 下约 13-15×，B=1 端到端 ~250×（详见 §7），4-8× 是市场观察到的 *售价* 比，**它小于** 纯成本比。定价在 *压缩* 成本差。**§7 / §9**。
...
> 上表所有 FP16 / FP8 / HBM 规格截至 2026-07-20 NVIDIA 公开 brief/datasheet 口径（B200 datasheet，FP8 / FP4 数字以 brief 为准，未到芯片级 benchmark 复核）。
...
B=128 在 FP16 下也只到 H100 ridge 的 10.5%——decode 工作点死死卡在屋顶线斜线段。
...
**总吞吐涨 ~20-40×，TPOT 涨 ~5×——这个不对称恰恰是 batching 划算的原因**：批处理把 GPU 的算力 / 带宽填得更满，但用户对单条请求的等待时间只线性涨。用户在线等不起，所以 serving 系统会有 "对话态小 batch、离线批处理大 batch" 两套配置，**平均单价可以差 10× 以上**。
...
每代 tensor core 升级都会把峰值拉一截，但峰值是 *特定精度* 下的（**所有数字截至 2026-07-20 NVIDIA 公开 brief/datasheet 口径**）：

- FP16 with FP32 accumulate：H100 1979 TFLOPs（密集），稀疏翻倍。
- FP8 (E4M3 / E5M2)：H100 3958 TFLOPs（密集）。
- FP4 (Blackwell 引入)：B200/GB200 ~9 PFLOPs dense / ~18 PFLOPs sparse。
...
按 §3.5 理想吞吐上限取代表工况（B=1 → 24 tok/s，B=32 → 400 tok/s，B=128 → 480 tok/s——量级取自 §4.1 表的公开 benchmark 中位），按 §3.6 input 公式（prefill ≈ 145 GFLOPs/token，MFU 45%）反推 input 吞吐：
...
| 场景 | 吞吐 (tok/s/GPU) | $/GPU·h | GPU·h/1M tok | **$ / 1M tok** |
|---|--:|--:|--:|--:|
| 在线 B=1 H100 FP16 (S=4096) | ~6000 | 2.4 (云) | 0.046 | **0.11** |
| 自建 B=1 H100 FP16 (S=4096) | ~6000 | 1.2 (自建) | 0.046 | **0.055** |

> 算式：prefill FLOPs = 2PS + 2LHS² ≈ 145 GFLOPs/token。H100 FP16 1979 TFLOPs，MFU 45% → 6140 tok/s。租价 $2.4/h → $0.109/1M input token。GB200 估测按 §3.5 FP16 理想 × 1.5×（FP8 增益）。
>
...
也就是说：**batched serving 下纯硅成本差 ~13-15×**；**B=1 端到端场景（无任何摊薄）拉到 ~250×**——取决于 batch / caching 摊薄程度。

### 7.2 换代卡的每美元账

H100 → H200 → B200 的 decode 吞吐收益等于带宽增幅本身（§2 已论证）。代入云租价后看每美元改善：

| 比对 | 带宽变化 | 吞吐变化 | 租价变化 | $/1M output 变化 | 净改善 |
|---|---:|---:|---:|---:|---:|
| H100 → H200, B=1 | +43% | +43% | $2.4 → $3-4 (+25-67%) | $27.8 → $31.6-39.6 | **-42% ~ +14%** |
| H100 → B200, B=1 | +139% | +139% | $2.4 → $5-8 (+108-233%) | $27.8 → $33.0-45.3 | **-63% ~ +19%** |
| H100 → H200, B=32 | +43% | +43% (INT4) | 同上 | $1.67 → $1.90-2.38 | **-42% ~ +14%** |

**每美元改善取决于供应商怎么定价**——带宽红利大致被租价吃掉甚至倒挂（H200/H100 单价关系主要看供应商怎么定价，不是硬件决定的）。Blackwell + MoE + 量化 + 良好 batching 才能把 "理论下限" 压到 $1/1M 量级。
...
- **output 比 input 贵 4-8×** 是数据复用结构的工程结果：prefill 摊薄权重和 attention，decode 不摊，rooftline 上 decode 永远卡在带宽侧。
