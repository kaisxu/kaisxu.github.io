...
B=128 都还没到 H100 ridge 的 11%。这就是 output token 工作的真实 "地形"：**永远坐在 roofline 的带宽侧**。
...
- "4-8× 价格差是物理常数"——纯硅成本差 batched serving 下约 6-8×，B=1 端到端 ~127×（详见 §7，**round 5 修正 sparse/dense 口径后从 ~13-15× 降到 6-8×**），4-8× 是市场观察到的 *售价* 比，与纯硅成本 **基本一致**——市场没有压缩成本差。**§7 / §9**。
- "H100 decode 时只跑出峰值算力的 0.5%"——结论跟口径有关：FP16 严格匹配时 H100 B=1 是 0.18%；换成 Blackwell FP4 tensor-core 峰值做分母会显得更难看；GQA 比 MHA 略好。**§3.5**。
- "PIM / UMA / LPU 是颠覆性突破"——这些方案都把内存层级往上推一阶，绕开 HBM 带宽墙。以 H100 的 HBM : FP16 dense = 3.35 TB/s : 989 TFLOPs = 1 : 295 (byte/FLOP) 为基准，新方案把这个比值升到 **1 : 2-6**——按 Groq LPU 实测 80 TB/s : 188 TFLOPs FP16 ≈ 1 : 2.3 算（**§8.3**）。这才是结构上的差异：把 HBM 的 "1 字节 1 FLOP 比" 改成 SRAM 的 "1 字节 ~0.5 FLOP"，让 ridge 远远推到工作点上方，decode 工作点终于能摸到算力墙。
...
> 上表所有 FP16 / FP8 / HBM 规格截至 2026-07-20 NVIDIA 公开 brief/datasheet 口径。**Sparse 营销值**：H100/H200 FP16 sparse = 1979 / FP8 sparse = 3958（A100 不支持 2:4 sparse）；B200/GB200 4th-gen Tensor Core 仍支持 2:4 sparse，FP16 sparse = 4500 / 5000。**Decode 永远不会跑到 sparse**：KV cache 是 FP16/FP8，attention 主路径不稀疏；权重 GEMM 能用 sparse 但需要先做 2:4 结构化剪枝后才能翻倍吞吐，70B 类预训练权重大多没剪。
...
- **其二，表中 "decode 利用率" 是诊断量，不能当性能指标读**——它衡量的是 "FP16 峰值被浪费了多少"，不是 "硬件升级带来了多少收益"。从 10.5% 到 22% 不是执行效率提升，是分母（用 sparse 还是 dense）变了；一旦 dense 一致口径，H100/B200 都是 ~21-22%，H200 反倒最高（30%，因为带宽涨但峰值没动）。
...
| 硬件 | H100 SXM 80GB（3.35 TB/s, 1979 TFLOPs FP16, 3958 TFLOPs FP8） | H200 / B200 已在 §2 列出 |
...
- **FP16 with FP32 accumulate**：
  ...
  - B200 / GB200：**2250 / 2500 TFLOPs dense** / **4500 / 5000 TFLOPs sparse**（4th-gen Tensor Core 仍支持 2:4 sparse）
...
但 decode 实际工作的是 FP16（部分 FP8）——**FP4 tensor core 几乎用不上**。即使 FP16，**decode 也永远不会跑到 sparse 路径**：KV cache 是 FP16/FP8（KV 数值取决于 softmax 后的 attention 输出，无法结构化剪枝），attention 主路径不稀疏；权重 GEMM 理论上能用 sparse 翻倍，但需要先做 2:4 剪枝（破坏 70B 类预训练权重的精度），实际 LLM serving 几乎不用。**标称的 1979 / 4500 是 sparse 营销值——真实能稳定跑到的工作峰值是 dense 989 / 2250；decode 连 dense 989 的 0.18%（B=1）都吃不到**。这正是 §2 observation 第一条想说的：标称 TFLOPs 对 decode 是无效指标，正确标尺是 TB/s × AI。
...
> 算式：prefill FLOPs = 2PS + 2LHS² ≈ 145 GFLOPs/token。H100 FP16 **dense 989 TFLOPs**（不用 sparse 1979：sparse inference 需 2:4 剪枝，prefill dense matmul 不走 sparse 路径；详见 §5.2），MFU 45% → 989 × 0.45 / 0.145 ≈ **3070 tok/s**。租价 $2.4/h → $0.217/1M input token。**注意**：本版（round 5 之后）input 端数字比 round 4 翻倍——之前用 sparse 1979 算 MFU 是口径错配（sparse peak 不对应 dense workload 的真实吞吐），修正后 input/output 纯硅比从虚高的 ~13-15× 降到 ~6-8×（详见 §7.3 第三条观察）。
>
...
| Together / Fireworks（跑开源 LLaMA-70B）[待核] | ~0.9 | ~0.9 | ~1× |

> **绝对价格口径警告**：GPT-4.1 / Claude / Gemini 是闭源 frontier 模型，参数 / 架构 / MoE 结构 / 量化都不公开，跟 LLaMA-70B 的硅账做绝对水平的对比是苹果对橘子。**out/in 比值**的论证仍然站得住（比值近似与模型规模无关，主要由 batch / caching 摊薄程度决定）——所以 4-8× 这一行的解读用 frontier 模型没问题。但**绝对 $/M** 的对照应优先用 Together / Fireworks 之类跑 LLaMA-70B 的服务商（量级 $0.9/M），它们的售价离 §7.1 算出的 batched output 纯硅成本 $1.39-1.67/M 最近。
...
3. **4-8× out/in 售价 ≈ batched serving 纯硅比（~6-8×）**——之前用 sparse 1979 TFLOPs 算 input MFU 把 input 成本低估约一半，导致 input/output 纯硅比虚高到 ~13-15×；修正成 dense 989 后降到 ~6-8×，**与公开 API 4-8× 售价比基本一致**。这意味着市场并没有在 "压缩" 成本差——延迟溢价通过 input 端的 cache-hit 摊薄（典型 80%+）、output 端的批处理摊薄双向吸收掉了，**硅物理和市场价 align 在 4-8× 这条线上**。这是 round 5 修正 sparse/dense 口径后最强的反直觉观察。
...
**思路**：让 CPU 和 GPU **访问同一块物理 HBM**（MI300A；Grace Hopper 走 NVLink-C2C，本质仍分层），去掉 PCIe/NVLink 跨设备拷贝。**对 decode 工作点：AI 不变**（仍卡 HBM 带宽墙），但**容量翻倍且 CPU/GPU coherent**——直接消掉 §5.5 的 KV 容量硬约束，单卡可服务序列数从 33 条（H100 INT4）推到 ~80 条（MI300A INT4），decode 实际 batch 能跑到理论上限的更大部分。
...
- **GPU 的算力墙和带宽墙之间有 gap**：ridge 在 280-590 FLOP/byte 之间，把 LLaMA-70B FP16 decode 工作点推到 **0.18% (B=1) ~ 22% (B=128)** 利用率（§2 最后一列）。要填这个 gap，要么提升带宽（昂贵但近几代在加速），要么改 AI（软件层）。
...
- **output 比 input 贵 4-8×** 是数据复用结构的工程结果：prefill 摊薄权重和 attention，decode 不摊，roofline 上 decode 永远卡在带宽侧。
- **公开 API 标价 $1-25 / 1M token**，梯度主要来自模型档位（DeepSeek V3 $1 到 Opus 4.5 $25）。同一档位下批处理越满、cache 命中率越高，单 token 实际成本越接近纯硅下界。
...
- **硬件升级的美元改善取决于供应商定价**：H200/B200 带宽红利大致被租价吃掉甚至倒挂；想拿到 "理论下限" 必须配 MoE + 量化 + 良好 batching + 大 batch。
