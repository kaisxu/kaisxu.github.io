...
注意表格里有两个完全不同的量：线性层 AI 是 $$2B/\sigma_w$$（按 batch 摊权重），attention 算术强度是 $$2g/\sigma_{kv} = 8$$（与 batch 无关、与上下文长度无关——**与 batch 无关**才是它卡带宽的根因：让 attention 摆脱带宽只能改 $$g$$ 和 $$\sigma_{kv}$$，即 GQA / FP8 KV）。前者解释了 batching 的红利，后者解释了为什么 batching 不能无限提升。第三节会从公式推导。
...
- "4-8× 价格差是物理常数"——纯硅成本差大约 **batched serving 下 13-15×，B=1 端到端 ~250×**（详见 §7），4-8× 是市场观察到的 *售价* 比，**它小于** 纯成本比。定价在 *压缩* 成本差。
- "H100 decode 时只跑出峰值算力的 0.5%"——结论跟口径有关：FP16 严格匹配时 H100 B=1 是 0.18%；换成 Blackwell FP4 tensor-core 峰值做分母会显得更难看；GQA 比 MHA 略好。
- "PIM / APU / LPU 是颠覆性突破"——这些方案都把内存层级往上推一阶，绕开 HBM 带宽墙。以 H100 的 HBM : FP16 = 3.35 TB/s : 1979 TFLOPs = 1 : 590 (byte/FLOP) 为基准，新方案把这个比值升到 **1 : 2-6**——按 Groq LPU 实测 80 TB/s : 188 TFLOPs FP16 ≈ 1 : 2.3 算（§8.3）。这才是结构上的差异。
...
| B | AI_agg (FLOP/byte) | 理想吞吐 — FP16 权重 | 理想吞吐 — INT4 权重 |
|---:|---:|---:|---:|
| 1 | 1.07 | **23.7** | **92** |
| 8 | 8.00 | 178 | 586 |
| 32 | 26.4 | 586 | 1376 |
...
> **重要 caveat**：上一版表格里我加了 "实测 tok/s" 和 "MBU = 实测/理想" 两列——结论是 B=1 ~100%、B=128 ~35%。这个漂亮曲线**是 artifact**：
>
> - 70B FP16 单卡物理放不下（§5.6）——所有公开 benchmark 都在 INT4 + TP ≥ 2 配置下跑；
> - FP16 "理想" 配 INT4 "实测" 等于用错分母，MBU 数字无意义；
> - 若严格按 INT4 + FP16 KV 重算理想（上面第四列），再除同样 INT4 实测值，MBU 会变成平坦的 ~20-30%（不是 B=1 满、B=128 跌）。
>
> **本节不报 MBU 数字**。Roofline 能保证的是 **AI_agg ≪ ridge，全部留在带宽段**——这是物理层面的硬结论，与实测是否打满无关。每个 B 下实测能跑到理想的几成由 attention kernel、KV layout、PCIe/NVLink 互联、SM occupancy 等综合决定，应直接查 [vLLM GitHub benchmark](https://github.com/vllm-project/vllm)，但**不要用他们的实测反推本文公式**——本文给出的是 "如果能跑出理想 AI 时的上限"。
...
> 上述数字是公开材料（vLLM / Anyscale / TGI / DeepSpeed-MII）的**典型量级**，因模型规模、量化、TP 配置不同而不严格可比——本文只关心 "TPOT vs 总吞吐" 的反向关系与 ~10× 单价差这一结论。**上一版的具体数字（24 / 120 / 400 / 480）来自不同 benchmark 的混合转引，不再保留。**

总吞吐涨 5-10 倍，但 TPOT 也涨 5-10 倍。对话态用户等不起，所以 serving 系统会有 "对话态小 batch、离线批处理大 batch" 两套配置，**平均单价可以差 10× 以上**。
...
- **OSL 大（长输出）**：attention 项对 $$o = 1..O$$ 求和后是 $$4LH \cdot O(O-1)/2$$（带下三角常数 1/2），整体 ≈$$O^2$$ 增长；线性层项线性增长，两项相加时 attention 主导。简单记忆："output 越长，attention 部分增长越快"。
...
S 和 O 都大时 prefill 仍是大头，但 attention 部分（OS + O(O-1)/2）随 O 二次增长——第三行 attention 已经占 decode 41%；如果继续往 O=32k、S=32k 推，attention 会进一步吃掉 decode 预算，**这部分也是 §7 公开 API 价目里某些 "长 output 单价非线性上涨" 的物理来源**。
...
带宽增长曲线在加速：H100 → H200 是 +43%（同代 8-Hi，只是频率提升），H100 → B200 是 +139%（更宽 stack + 频率）。HBM3e 12-Hi 路径已在 B300 上落地（288 GB / 卡），对应的核心瓶颈不是物理堆叠，而是良率与 TSV 工艺。**单卡相对 H100 已 +139%，要充分发挥（H200/B200 之间的 +52%）还得靠 NVLink Switch + 72-way NVL72 横向扩展**——那是另一条独立的扩展路线，与 HBM 单卡带宽不在一个维度上度量。
...
| **量化（FP8 / INT8 / INT4 权重）** | 否 | σ_w ↓ | 线性层 AI_lin = 2B/σ_w，同样 B 下随 σ_w 下降而涨 |
...
|---|---|---:|---:|---:|
| B=1 对 B=1 | 实时对话 H100 FP16 | 11.57 | 0.046 | **~250×** |
| B=32 对 B=1 | 在线批量 H100 INT4 / 云 input FP16 | 0.694 | 0.046 | **~15×** |
| B=128 对 B=1 | 离线批量 H100 INT4 / 云 input FP16 | 0.579 | 0.046 | **~13×** |
| B=32 自建 | 自建 output / 云 input (跨 $/h) | 0.694 | 0.046 | ~15×（约掉后同理）|

也就是说：**batched serving 下纯硅成本差 ~13-15×**；**B=1 端到端场景（无任何摊薄）拉到 ~250×**。§1.1 那个粗口径 "8-25× 纯硅成本差" 在认真算之后应改成 **~13-250×**（取决于 batch / caching 摊薄程度）。
...
3. **4-8× out/in 售价比 < 纯硅成本比**。如果只算硅，batched serving 下 output 单 token 应比 input 贵 ~13-15×；售价只标 4-8×，说明供应商定价 *压缩* 了成本差。这一压缩来自 input 端按 cache-hit 摊薄（典型调用 system prompt 命中率 ≥ 80%，实际入账 token 量远低于理论 input）、output 端按批处理摊薄的双向作用。
...
### 8.2 统一内存架构（UMA — Unified Memory Architecture）——把 CPU/GPU 边界打破
...
- MoE、speculative decoding、FP8 KV、prefix caching 是目前最有效的几条——它们各自在压低每 token 的字节数或计算量（哪个改 FLOPs、哪个改字节、哪个改容量见 §6 表）。Linear attention、状态空间模型是从根本上让 attention 项不再随 T 二次增长的方向。
- **硬件升级对单 token 单价的美元改善取决于供应商怎么定价**：H200 带宽 +43% → B=1 decode 吞吐按带宽比例 +43% → 理论 $27.8 → $19.4/1M output（改善 30%）；云租价在 $3-4/h（+25-67%）之间，对应 **净改善落在 -17% ~ +13%**——带宽红利被租价吃掉一半以上甚至倒挂（H200 比 H100 还贵），单 token 单价关系主要看供应商怎么定价。Blackwell + MoE + 量化 + 良好 batching 才能把 "理论下限" 压到 $1/1M 量级。
