...
观察：H200 把带宽加了 43%，但 FP16 ridge *反而下降*（从 590 到 412），因为峰值算力没变。Blackwell 把带宽推到 8 TB/s，ridge 进一步降到 ~280。**从 ridge 公式的数学形式看，ridge 下降是 "算力峰没动 + 带宽涨" 的派生结果；对 decode 工作点（AI_agg ≈ 62）来说，ridge 在 281 还是 590 都没影响——工作点离 ridge 始终差 5-10 倍。**

那 H200/B200 对 decode 究竟意味着什么？看最后一列：H100 → H200 → B200 的 decode 利用率从 10.5% → 15% → 22%，**直接来自带宽绝对值提升**（按 §3 公式，B=128 聚合 AI ≈ 62 FLOP/byte，乘上 3.35 / 4.8 / 8.0 TB/s 即可反推可达吞吐）。但云租价同步上涨（$2.4/h → $3-4/h → $5-8/h），所以**每美元改善远小于带宽提升**——这是 §9 的量化结论。
